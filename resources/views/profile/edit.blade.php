...
@section('title', __('app.manage_profile'))
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom border-light">
        <div>
            <h2 class="fw-bold text-dark mb-1 fs-4">{{ __('app.profile_settings') }}</h2>
            <p class="text-muted small mb-0">{{ __('app.profile_settings_subtitle') }}</p>
        </div>
    </div>
    ...
    <div class="row g-4">
        <div class="col-12 col-xl-8">
            ...
            <div class="card border-0 shadow-sm bg-white p-4 rounded-3 mb-4">
                <h5 class="fw-bold mb-4">{{ __('app.profile_picture') }}</h5>
                <div class="d-flex align-items-center gap-4">
                    ...
                    <div class="position-relative">
                        ...
                        <label for="avatarInput"
                               ...
                               style="width:30px;height:30px;cursor:pointer;" title="{{ __('app.change_photo') }}">
                    ...
                    <div>
                        ...
                        <div class="text-muted small mb-3">
                            {{ \Illuminate\Support\Facades\Lang::has('app.role_' . auth()->user()->role, app()->getLocale())
                        ...
                        <div class="d-flex gap-2">
                            <label for="avatarInput" class="btn btn-sm btn-outline-primary">
                                <i class="bi bi-upload me-1"></i>{{ __('app.upload_photo') }}
                            </label>
                            @if($user->avatar)
                            <form action="{{ route('avatar.destroy') }}" method="POST">
                                ...
                                <button class="btn btn-sm btn-outline-danger"
                                        onclick="return confirm('{{ __('app.confirm_remove_photo') }}')">
                                    <i class="bi bi-trash me-1"></i>{{ __('app.remove') }}
                                </button>
                            </form>
                            @endif
                        </div>
                        <small class="text-muted d-block mt-2">{{ __('app.avatar_constraints') }}</small>
                    </div>
                </div>
            </div>
